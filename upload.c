...
//	int binBuf[4096],isBin=1;//contBuf[MAX_FILE_SIZE] is too large and segment err
	...
	//path is :/upload#<dir>
	sprintf(realpath,"%s%s/%s",webpage_root,&path[8],filename);
...
"function fun() { if (i == 0) {window.history.back(-1); "//window.location.href =\"..\";" 
"clearInterval(intervalid); }i--;}" 
"</script> <body><center><h1>Upload success!</h1><br>will return home page in 5 secs.</center></body></html>\r\n\r\n";
